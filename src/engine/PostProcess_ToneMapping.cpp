...
void PostProcessToneMapping::BindDescriptorSets(VkCommandBuffer cmdBuffer, VkDescriptorImageInfo sampleImageInfo)
{
	// 更新描述符集
	{
		...
		vks::initializers::writeDescriptorSet(descriptorSet[vkUtils::vkEngine->currentBuffer], VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &sampleImageInfo)
		};
...
void PostProcessToneMapping::preparePipeline()
{
	...
	builder.addShaderStage(vkUtils::vkEngine->loadShader(vkUtils::vkEngine->getShadersPath() + "PostProcess_toneMapping.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT));
	VkPipelineRenderingCreateInfo renderInfo = vks::initializers::pipelineRenderingCreateInfo(1, &vkUtils::vkEngine->swapChain.colorFormat);
