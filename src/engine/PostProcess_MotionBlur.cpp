...
void PostProcessMotionBlur::setParams(float maxBlurRadius, float depthThreshold, float alphaWeightScale)
{
	...
	memcpy(paramBuffer[vkUtils::vkEngine->currentBuffer].mapped, &params, sizeof(MotionBlurConstants));
}
...
void PostProcessMotionBlur::BindDescriptorSets(VkCommandBuffer cmdBuffer, VkDescriptorImageInfo colorImageInfo, VkDescriptorImageInfo depthImageInfo)
{
	// 更新描述符集
	{
		...
		std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
			vks::initializers::writeDescriptorSet(descriptorSet[vkUtils::vkEngine->currentBuffer], VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &paramBuffer[vkUtils::vkEngine->currentBuffer].descriptor),
...
void PostProcessMotionBlur::preparePipeline()
{
	...
	builder.addShaderStage(vkUtils::vkEngine->loadShader(vkUtils::vkEngine->getShadersPath() + "PostProcess_MotionBlur.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT));
